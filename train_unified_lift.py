...
@torch.no_grad()
def update_code_usage(code_usage, virtual_gt_labels, confidence_map, momentum=0.99):
    """EMA tracking of how much each codebook entry is used by confident pixels.

    This is the soft-assignment "mass" signal that drives the adaptive codebook:
    entries that keep receiving (almost) no confident pixels are treated as dead.
    """
    num_codes = code_usage.shape[0]
    conf_mask = (confidence_map > 0.5).reshape(-1)
    if conf_mask.sum() == 0:
        return code_usage
    used = virtual_gt_labels.reshape(-1)[conf_mask]
    hist = torch.bincount(used, minlength=num_codes).float()
    hist = hist / (hist.sum() + 1e-6)
    code_usage.mul_(momentum).add_((1.0 - momentum) * hist)
    return code_usage


@torch.no_grad()
def revive_codebook(code_book, optimizer, code_usage, dead_ratio=0.01, noise_scale=0.1):
    """VQ-VAE-style codebook revival / splitting (Idea 1: adaptive codebook).

    Dead codes (usage far below the mean) are not pruned away to shrink the
    tensor; instead they are *revived* by splitting an over-loaded ("donor")
    code into two perturbed children. The donor is sampled with probability
    proportional to its usage, so high-load objects that should be split into
    finer instances get the extra capacity, while truly unused capacity is
    recycled. This lets the effective number of objects be discovered from the
    data instead of staying fixed at the full codebook size.

    Returns the number of revived codes.
    """
    mean_usage = code_usage.mean()
    dead = code_usage < (mean_usage * dead_ratio)
    n_dead = int(dead.sum().item())
    if n_dead == 0 or n_dead == code_usage.numel():
        return 0

    donor_w = code_usage.clone()
    donor_w[dead] = 0.0
    if donor_w.sum() <= 0:
        return 0

    dead_idx = torch.nonzero(dead, as_tuple=False).squeeze(1)
    donor_idx = torch.multinomial(donor_w, n_dead, replacement=True)

    donors = code_book.data[donor_idx]
    noise = torch.randn_like(donors) * noise_scale * donors.norm(dim=-1, keepdim=True)
    # split each donor into two diverging children
    code_book.data[dead_idx] = donors + noise
    code_book.data[donor_idx] = donors - noise

    # reset Adam moments for the rows we just overwrote so stale momentum
    # does not drag the freshly seeded codes back to their old positions
    state = optimizer.state.get(code_book, None)
    if state is not None and 'exp_avg' in state:
        state['exp_avg'][dead_idx] = 0.0
        state['exp_avg_sq'][dead_idx] = 0.0
        state['exp_avg'][donor_idx] = 0.0
        state['exp_avg_sq'][donor_idx] = 0.0

    # split the usage mass between donor and child
    half = code_usage[donor_idx] * 0.5
    code_usage[dead_idx] = half
    code_usage[donor_idx] = half
    return n_dead


def compactness_loss_3d(feat3d, xyz, code_book, sample_size=50000, temperature=0.1, eps=1e-6):
    """3D instance compactness loss (Idea 2).

    Each codebook entry should correspond to a spatially *compact* set of
    Gaussians. We soft-assign every (sub-sampled) Gaussian to the codebook via
    its per-Gaussian feature, then penalise the assignment-weighted spatial
    variance of each code. This directly punishes floaters / split objects that
    share a code but sit far apart in 3D -- a signal that pure 2D supervision
    cannot provide. xyz is detached so the loss shapes the features/codebook,
    not the geometry (which is owned by the RGB reconstruction).
    """
    N = feat3d.shape[0]
    if N > sample_size:
        idx = torch.randperm(N, device=feat3d.device)[:sample_size]
        feat3d = feat3d[idx]
        xyz = xyz[idx]

    feat3d = feat3d / (feat3d.norm(dim=-1, keepdim=True) + eps)
    cb = code_book / (code_book.norm(dim=-1, keepdim=True) + eps)

    p = torch.softmax(torch.matmul(feat3d, cb.T) / temperature, dim=-1)  # (N, K)
    xyz = xyz.detach()

    w = p.sum(dim=0) + eps                       # (K,) per-code mass
    centroid = (p.T @ xyz) / w.unsqueeze(-1)     # (K, 3)
    sq = (xyz * xyz).sum(dim=-1)                 # (N,)
    mean_sq = (p.T @ sq) / w                      # (K,)
    var = (mean_sq - (centroid * centroid).sum(dim=-1)).clamp(min=0)  # (K,)

    # mass-weighted mean variance, normalised by the scene spread so the loss
    # is scale-invariant across scenes
    loss = (var * w).sum() / w.sum()
    global_var = (sq.mean() - (xyz.mean(dim=0) ** 2).sum()).clamp(min=eps)
    return loss / global_var


def training(dataset, opt, pipe, testing_iterations, saving_iterations, checkpoint_iterations, checkpoint, debug_from, use_wandb, weight_loss, weight_3d, reg3d_from, revive_interval):
    first_iter = 0
    ...
    # Adaptive codebook (Idea 1): EMA usage of each code, and the window in which
    # we allow revival/splitting (warm up first, then freeze near the end so the
    # codebook stabilises before the final assignment).
    num_codes = code_book.shape[0]
    code_usage = torch.zeros(num_codes, device="cuda")
    revive_start = max(reg3d_from, 2000)
    revive_end = int(0.8 * opt.iterations)
    if checkpoint:
    ...
    for iteration in range(first_iter, opt.iterations + 1):        
        ...
        # Adaptive codebook (Idea 1): track per-code usage from confident pixels.
        update_code_usage(code_usage, virtual_gt_labels, confidence_map)
        ...
        # 3D instance compactness loss (Idea 2): penalise spatially spread-out
        # assignments of each codebook entry over the per-Gaussian features.
        if iteration > reg3d_from:
            feat3d = gaussians._objects_dc.squeeze(1)   # (N, 16)
            compactness_loss = compactness_loss_3d(feat3d, gaussians._xyz, code_book)
        else:
            compactness_loss = torch.tensor(0., device=logits.device, requires_grad=True)
        ...
        if False:
            ...
            loss = (1.0 - opt.lambda_dssim) * Ll1 + opt.lambda_dssim * (1.0 - ssim(image, gt_image)) + contrastive_loss * 1e-6  + loss_obj_cls * 1e-4  + clustering_loss * weight_loss +  regularization  * 1e-6 + compactness_loss * weight_3d
        ...
        with torch.no_grad():
            ...
            if use_wandb:
                wandb.log({"compactness_loss": float(compactness_loss), "iter": iteration})
            ...
            if iteration < opt.iterations:
                ...
                # Adaptive codebook (Idea 1): periodically recycle dead codes by
                # splitting over-loaded ones, within the revival window.
                if revive_interval > 0 and revive_start < iteration < revive_end \
                        and iteration % revive_interval == 0:
                    n_revived = revive_codebook(code_book, cls_optimizer, code_usage)
                    if n_revived > 0:
                        n_active = int((code_usage > code_usage.mean() * 0.01).sum().item())
                        print("\n[ITER {}] Codebook revived {} codes ({} active)".format(
                            iteration, n_revived, n_active))
                        if use_wandb:
                            wandb.log({"codebook/revived": n_revived,
                                       "codebook/active": n_active, "iter": iteration})
...
if __name__ == "__main__":
    ...
    parser.add_argument("--weight_3d", type=float, default=0.1, help="Weight of the 3D instance compactness loss (Idea 2)")
    parser.add_argument("--reg3d_from", type=int, default=3000, help="Iteration to start the 3D compactness loss / codebook revival warmup")
    parser.add_argument("--revive_interval", type=int, default=1000, help="Interval (iters) for adaptive codebook revival; 0 disables (Idea 1)")
    ...
    training(lp.extract(args), op.extract(args), pp.extract(args), args.test_iterations, args.save_iterations, args.checkpoint_iterations, args.start_checkpoint, args.debug_from, args.use_wandb, args.weight_loss, args.weight_3d, args.reg3d_from, args.revive_interval)
